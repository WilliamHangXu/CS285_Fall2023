    ...
    for itr in range(params["n_iter"]):
        ...
        if itr == 0:
            ...
            if params["do_dagger"]:
                ...
                for step in paths:
                    step["action"] = expert_policy.get_action(step["observation"])
        ...
        for _ in range(params["num_agent_train_steps_per_iter"]):
            ...
            ob_batch = torch.from_numpy(replay_buffer.obs[idx]).to(ptu.device)
            ac_batch = torch.from_numpy(replay_buffer.acs[idx]).to(ptu.device)
